...
def xzCloudPlot(nest,datetuple,latlon,plotTemp=False,plotRH=False,plotTheta=True, plotWind=True):
    ...
    xs_angle = N.arctan((yy[-1]-yy[0])/(xx[-1]-xx[0])) # In radians
    ...
    if plotWind == True:
        ...
        u_xz = nc.variables['U'][time,:,yy.astype(N.int),xx.astype(N.int)-1]
        v_xz = nc.variables['V'][time,:,yy.astype(N.int),xx.astype(N.int)]
        wind_xz = N.sqrt(u_xz**2 + v_xz**2)
        # Decompose into plane-parallel
        planewindf = N.asarray([math.atan2(v,u) for v,u in zip(v_xz.flatten(),u_xz.flatten())])
    ...
    plt.contourf(grid, heighthalf_xz, u_xz, alpha=0.6, levels=u_wind_levels,cmap=plt.cm.jet)
    ...
    plt.plot(arange(hypotenuse),heightground_x,color='black')
    plt.fill_between(arange(hypotenuse),heightground_x,0,facecolor='lightgrey')
    #plt.xticks(np.arange(0,hypotenuse,20),np.round(longitude[Ny/2,::8], 1), fontsize='small')
