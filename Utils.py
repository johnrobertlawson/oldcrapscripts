...
def convert_time(nest,datetuple): # JRL function
    # This converts a datestring to a timestep in the WRF file
    nc = openWRF(nest)
    times = nc.variables['Times'][:]
    
    # This converts datestring into Python time
    yr,mth,day,hr,min,sec = datetuple # For plot labelling, potentially
    usertime = cal.timegm(datetuple)
    
    # This converts WRF times into Python times
    wrftime = []
    for i,_i in enumerate(times):
        wrfyr = int(''.join(times[i][0:4]))
        wrfmth = int(''.join(times[i][5:7]))
        wrfday = int(''.join(times[i][8:10]))
        wrfhr = int(''.join(times[i][11:13]))
        wrfmin = int(''.join(times[i][14:16]))
        wrfsec = int(''.join(times[i][17:19]))
        wrftime.append(cal.timegm([wrfyr,wrfmth,wrfday,wrfhr,wrfmin,wrfsec]))

    # This find the nearest WRF time to desired time stamp
    gap = np.array([abs(w-usertime) for w in wrftime])
    WRFtimeloc = (list(gap)).index(gap.min())
    print "The desired date/time in WRF output time #", WRFtimeloc
    return WRFtimeloc
...
def openWRF(nest):
    ...
    file_wrf = directory + 'wrfout_d0' + str(nest) + '_' + date
...
def getXYxs(lon,lat,ptll): # JRL edit
    ptlat, ptlon = ptll
    x_nr = 0
    y_nr = 0
    while (lon[x_nr] < ptlon):
        x_nr += 1
    while (lat[y_nr] < ptlat):
        y_nr += 1    
